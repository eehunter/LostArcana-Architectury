...
class BasicSalisMundisTransformRecipe(val block: Block, val result: ItemStack): SalisMundisTransformRecipe() {
    ...
    object Serializer: RecipeSerializer<BasicSalisMundisTransformRecipe>{
        val CODEC: Codec<BasicSalisMundisTransformRecipe> = RecordCodecBuilder.create {
            it.group(
        ...
        val MAP_CODEC: MapCodec<BasicSalisMundisTransformRecipe> = MapCodec.assumeMapUnsafe(CODEC)

        override fun codec(): MapCodec<BasicSalisMundisTransformRecipe> = MAP_CODEC

        val STREAM_CODEC: StreamCodec<RegistryFriendlyByteBuf, BasicSalisMundisTransformRecipe> = StreamCodec.of<RegistryFriendlyByteBuf, BasicSalisMundisTransformRecipe>({ buf, recipe ->
